window.vxgplayer = function (id, options) {
	window.vxgplayer.version = "2.8.22"; //version is updated by 'npm run build'
	...
	if (!window.vxgplayer.players[id]) {
		...
		if (!vxgplayer.browserSupportsPluginPnacl()) {
			...
			if (fs) {
				fs(window.TEMPORARY, 100, function () {}, function () {
					vxgplayer.showNotInstalledInIncognitoMode(id);
				});
			}
			return undefined;
		}
		...
		if (window.location.protocol != "https:" && !vxgplayer.isFrame()) {
			vxgplayer.checkWebSocket().done(function (result) {
				...
				fs(window.TEMPORARY, 1, function () {
					vxgplayer.showWebSocketFailed(id);
				}, function () {
					vxgplayer.showNotStartedInIncognitoMode(id);
				});
			});
		}
		...
		window.vxgplayer.players[id] = new function (id, options) {
			...
			var nmf_src = "scripts/pnacl/Release/media_player.nmf";
			var url = "";
			var width = $(".livevideo").width();
			var height = $(".livevideo").innerHeight() - $(".selectcam").innerHeight();
			self.playerWidth = options.width || width;
			self.playerHeight = options.height || height;
				...
				if (msgEvent.data[0].charAt(0) == '@') {
					...
					if (0 != snap_status.indexOf("ERROR")) {
						...
						window.requestFileSystem(window.TEMPORARY, 1, function (fs) {
							fs.root.getFile(self.m.snapshotFile, {
								create: false
							}, function (fileEntry) { // /test is filename
