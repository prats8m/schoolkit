window.vxgplayer = function (id, options) {
	...
	if (!window.vxgplayer.players[id]) {
		...
		window.vxgplayer.players[id] = new function (id, options) {
			...
			var url = "";
			self.playerWidth = options.width || $(".livevideo").width();
			self.playerHeight = options.height || $(".livevideo").innerHeight() - $(".selectcam").innerHeight();
			...
			function connectToApp() {
				...
				self.module.port.postMessage({
					id: "" + self.module.port.name,
					cmd: "getversionapp",
					data: ""
				});
