...
app
    .controller('UserCtrl', function ($stateParams, $scope, $mdDialog, $http, $rootScope, $cookies, arrayPushService, $location, toaster, baseURL, $timeout, appConstants, userSvc, utilitySvc, $uibModal, $filter) {
        ...
        //Update credential after schedule add page
        $scope.editCredentialAfterSchuduleUpdate = function (cred_data, credential_type, ev) {
          switch (credential_type) {
            case 'access_code':
                ...
                $scope.submitData.user_id = parseInt($cookies.get("newUserId"));
                ...
                case appConstants.adduser:
                    switch (objectType) {
                        ...
                        case 'phoneCode':
                            ...
                            $scope.phoneCode.phoneCode_size = 5;
                            $rootScope.schedule.schedule_type = 'ONETIME';
                            break;
                        case 'rfid':
                            $scope.rfid = {};
        ...
        //Delete schedule credentials on add
        ...
        $scope.editCredential = function (cred_data, credential_type) {
            switch (credential_type) {
                ...
                case 'phone_code':
                    ...
                    angular.forEach(cred_data.Detail.phone_numbers, function(value, key) {
                      if(value.phone_number && key == 0){
                        $scope.phoneCode.phone_numbers1 = value.phone_number;
                        $scope.phoneCode.endtime1 = value.endtime;
                        $scope.phoneCode.starttime1 = value.starttime;
                      }
                      if(value.phone_number && key == 1){
                        $scope.phoneCode.phone_numbers2 = value.phone_number;
                        $scope.phoneCode.endtime2 = value.endtime;
                        $scope.phoneCode.starttime2 = value.starttime;
                      }
                      if(value.phone_number && key == 2){
                        $scope.phoneCode.phone_numbers3 = value.phone_number;
                        $scope.phoneCode.endtime3 = value.endtime;
                        $scope.phoneCode.starttime3 = value.starttime;
                      }
                    });
    ...
    .controller('UserProfileCtrl', function ($scope, $http, $cookies, $stateParams, baseURL, $rootScope, $location, toaster, $timeout, $mdDialog, $filter, appConstants, userSvc, utilitySvc) {
        ...
        $scope.submitEditWIEGANDCode = function (submitData, wiegand_form) {
            ...
            if ($scope.edit_wiegand_schedule.schedule.length == 0) {
                ...
                if ($rootScope.schedule.date != undefined) {
                    var start_date = new Date($rootScope.schedule.date);
